...
void TiltHexa_Pipeline::run_full_pipeline(
    ...
    alloc_out.solver_time_us = sensor->micros_now - t_start;
                ...
                // Run QP solver (constraints binding OR significant Fx in transition)
                // Use PI solution as warm start
                TiltHexa_ConstraintSet cs_combined;
                cs_combined.clear();
                thx_combine_rate_constraints(&cs_combined, &cs_pos, &cs_rate, pi_res.u);
                ...
                TiltHexa_QPResult qp_res = _qp_solver.solve(_params.qp_max_iter);
                if (qp_res.iterations == 0) qp_res.iterations = 1;
                alloc_out.solver_status = qp_res.status;
                alloc_out.solver_iterations = qp_res.iterations;
                alloc_out.n_active_constraints = qp_res.n_active;
                ...
                if (qp_res.status == THX_SOLVER_OK) {
                    ...
                    // QP failed (numerical or max iterations).
                    // ALWAYS fall back to PI solution rather than
                    // freezing _u_prev_vec at a stale value, which
                    // guarantees a growing wrench error and divergence.
                    alloc_out.solver_status = qp_res.status;
                    for (int i = 0; i < 16; i++) {
                        _u_prev_vec[i] = pi_res.u[i];
                    }
        ...
        if (!use_pi_directly) {
            // solver_time_us already set above for QP case
        }
    } else {
        // ---- Weighted PI + clipping (fallback to QP on large Fx) ----
        // Enable symmetric tilt for PI baseline: all rotors use the same
        // tilt angle; moments come from thrust differential.  This
        // prevents asymmetric-tilt-induced roll divergence at large Fx.
        _pi_solver.use_symmetric_tilt = true;
        // PI pseudo-inverse with per-motor clipping cannot handle large
        // Fx demands at high speed because the rate-limited tilt angles
        // produce uncoordinated wrench components.  Fall back to QP
        // (with proper constraint enforcement) when Fx demand exceeds
        // a small fraction of hover thrust.
        float Fx_demand = fabsf(alloc_in.w_d.Fx);
        float Fz_hover = _params.mass_kg * (_params.g > 0.0f ? _params.g : GRAVITY);
        bool significant_Fx = (Fx_demand > 0.05f * Fz_hover);
        ...
        if (!significant_Fx) {
            ...
            // Large Fx: use QP with proper constraint enforcement.
            // PI solution provides the warm-start point.
            TiltHexa_PIResult pi_res = _pi_solver.solve(&alloc_in);
            ...
            if (qp_res.status == THX_SOLVER_OK) {
                ...
                // QP failed (numerical or max iterations).
                // Always fall back to PI solution -- never freeze
                // _u_prev_vec at a stale value while w_d keeps changing.
                alloc_out.solver_status = qp_res.status;
                for (int i = 0; i < 16; i++) {
                    _u_prev_vec[i] = pi_res.u[i];
                }
    ...
    alloc_out.solver_time_us = sensor->micros_now - t_start;  // measured solve time
