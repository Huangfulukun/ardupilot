...
def run_bench(alloc="pi", mission="hover", alt=5.0, cruise=20.0, duration=40.0,
    ...
    truth_csv = os.path.join(out_dir, f"bench_{mission}_{alloc}_truth.csv") if out_dir else None
    if out_dir:
        os.makedirs(out_dir, exist_ok=True)
    fdm = TiltHexaFDM(config_path, instance=0, seed=seed, wind_ned=wind_ned, csv_out=truth_csv)
    ...
    if out_dir:
        os.makedirs(out_dir, exist_ok=True)
        csv_path = os.path.join(out_dir, f"bench_{mission}_{alloc}.csv")
